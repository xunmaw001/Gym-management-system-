...
<div :style='{"padding":"20px 20px 40px","margin":"10px auto 0","borderColor":"#52b646","borderRadius":"12px","background":"#fff","borderWidth":"3px","width":"1200px","position":"relative","borderStyle":"solid dashed double double"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="教练编号" prop="jiaolianbianhao">
              <el-input v-model="ruleForm.jiaolianbianhao" placeholder="教练编号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="教练名称" prop="jiaolianmingcheng">
            <el-input v-model="ruleForm.jiaolianmingcheng" 
                placeholder="教练名称" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="教练照片" v-if="type!='cross' || (type=='cross' && !ro.jiaolianzhaopian)" prop="jiaolianzhaopian">
            <file-upload
            tip="点击上传教练照片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="教练照片" prop="jiaolianzhaopian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiaolianzhaopian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="教练性别" prop="jiaolianxingbie">
            <el-select v-model="ruleForm.jiaolianxingbie" placeholder="请选择教练性别"  >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="教练特长" prop="jiaoliantezhang">
            <el-input v-model="ruleForm.jiaoliantezhang" 
                placeholder="教练特长" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="教练简介" prop="jiaolianjianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="教练简介"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="教练详情" prop="jiaolianxiangqing">
            <editor 
...
<script>
  export default {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jiaolianzhaopian'){
              this.ruleForm.jiaolianzhaopian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('jiaolianxinxi/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('jiaolianxinxi/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('jiaolianxinxi/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      jiaolianzhaopianUploadChange(fileUrls) {
          this.ruleForm.jiaolianzhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
