	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/bd196cf1072f48afae5676066a72e297.jpg) fixed no-repeat center top / 100% 100%"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.qicaibianhao" label="器材编号" prop="qicaibianhao">
					<el-input v-model="ruleForm.qicaibianhao" placeholder="器材编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.qicaizhaopian" label="器材照片" prop="qicaizhaopian">
					<file-upload
						tip="点击上传器材照片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.qicaizhaopian?ruleForm.qicaizhaopian:''"
						@change="qicaizhaopianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.qicaizhaopian" label="器材照片" prop="qicaizhaopian">
					<img v-if="ruleForm.qicaizhaopian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.qicaizhaopian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.qicaizhaopian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="器材用途" prop="qicaiyongtu">
					<el-input v-model="ruleForm.qicaiyongtu" placeholder="器材用途" clearable  :readonly="ro.qicaiyongtu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="器材用途" prop="qicaiyongtu">
					<el-input v-model="ruleForm.qicaiyongtu" placeholder="器材用途" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="器材详情" prop="qicaixiangqing">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.qicaixiangqing" label="器材详情" prop="qicaixiangqing">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#666","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.qicaixiangqing"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='qicaizhaopian'){
							this.ruleForm.qicaizhaopian = obj[o];
							this.ro.qicaizhaopian = true;
							continue;
						}
						if(o=='qicaiyongtu'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.qicaixiangqing = this.ruleForm.qicaixiangqing.replace(reg,'../../../ssmbgnk6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.qicaizhaopian!=null) {
		this.ruleForm.qicaizhaopian = this.ruleForm.qicaizhaopian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    qicaizhaopianUploadChange(fileUrls) {
	    this.ruleForm.qicaizhaopian = fileUrls;
    },
  }
};
</script>
